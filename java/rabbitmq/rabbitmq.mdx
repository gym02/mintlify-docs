...
2）**消费者**：用手动 ack，业务真正成功才 `basicAck`，处理成功以后发送 ack 给 Broker，Broker 才删消息；失败就 `basicNack`/`basicReject`，消息重新入队或进死信队列，再重试或人工处理。  
...
**补充：死信队列怎么用？有哪些场景？（面试口述版）**  
**怎么用（记住是在“业务队列”上配）：**  
- 声明业务队列时配置 `x-dead-letter-exchange`（可选再配 `x-dead-letter-routing-key`），指定死信交换机和路由键。  
- 按需再配 `x-message-ttl`（消息/队列 TTL）、`x-max-length` / `x-max-length-bytes` 等。  
- 当消息满足任一条件时会变成“死信”，被 DLX 路由到死信队列：  
  1）消息在队列里 **TTL 过期**；  
  2）队列长度/大小超出 `x-max-length` / `x-max-length-bytes` 被丢弃；  
  3）消费者 `basicReject` / `basicNack(requeue=false)` 拒绝且不重回原队列。  

**典型使用场景（好记 3 个）：**  
- **延迟 / 重试**：用 TTL + DLX 实现延迟队列、延迟重试（消息先在业务队列等过期，再流入死信队列，由专门消费者按需处理）。  
- **隔离“毒”消息**：处理多次失败或格式异常的消息，通过 `basicNack(requeue=false)` 直接打入死信队列，避免一直重试把主队列堵死。  
- **积压 & 告警**：大批过期/被拒的消息集中到死信队列，方便做监控、告警和人工排查，而不是静悄悄地丢失。  

**面试一句话：** 死信队列是给“处理不了/不再处理”的消息单独开的一条出口，通过在业务队列上配置 DLX + TTL/长度限制/拒绝策略，把异常消息集中到一个队列做延迟、重试或人工处理，避免堵住主业务队列。
